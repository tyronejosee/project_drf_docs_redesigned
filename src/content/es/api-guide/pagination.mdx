---
title: "Paginación"
keywords: ["django", "rest", "framework", "api", "paginación"]
order: 17
---

# Paginación

> Django proporciona algunas clases que te ayudan a gestionar datos paginados - es decir, datos que se dividen en varias páginas, con enlaces "Anterior/Siguiente".
>
> &mdash; [Django documentation][cite]

REST framework incluye soporte para estilos de paginación personalizables. Esto te permite modificar cómo los conjuntos de resultados grandes se dividen en páginas individuales de datos.

La API de paginación puede soportar:

- Enlaces de paginación que se proporcionan como parte del contenido de la respuesta.
- Enlaces de paginación que se incluyen en los encabezados de respuesta, como `Content-Range` o `Link`.

Los estilos incorporados actualmente usan enlaces incluidos como parte del contenido de la respuesta. Este estilo es más accesible cuando se usa la API navegable.

La paginación solo se realiza automáticamente si estás usando las vistas genéricas o viewsets. Si estás usando un `APIView` regular, necesitarás llamar a la API de paginación tú mismo para asegurar que devuelves una respuesta paginada. Consulta el código fuente para las clases `mixins.ListModelMixin` y `generics.GenericAPIView` para un ejemplo.

La paginación puede desactivarse estableciendo la clase de paginación en `None`.
